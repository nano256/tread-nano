...
def edm_sampler(
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
        ...
        # Euler step.
        denoised = net(x_hat.float(), t_hat, class_labels.long(), cfg_scale, feat=feat)['x'].to(torch.float64)
        d_cur = (x_hat - denoised) / t_hat
        x_next = x_hat + (t_next - t_hat) * d_cur

        # Apply 2nd order correction.
        if i < num_steps - 1:
            denoised = net(x_next.float(), t_next, class_labels.long(), cfg_scale, feat=feat)['x'].to(torch.float64)
